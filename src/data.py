...
class DataloaderFactory:
    ...
    def __call__(self, cfg, split_indexes, mode, path_root, shuffle, drop_last=False, transform=None):
        if cfg['dataset_name']=='DatasetPerImg':
            dataset = DatasetPerImg(path_root, split_indexes, mode, transform)
            dataloader = DataLoader(dataset, batch_size=cfg['batch_size'], shuffle=shuffle, drop_last=drop_last)
            return dataloader
...
class DatasetPerImg(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        annotation = self.df[['ID', 'LYMPH_COUNT', 'AGE', 'BIN_GENDER', 'LABEL']].loc[idx_patient].to_dict()
