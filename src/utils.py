...
def get_stratified_split(df_annotation_train, df_annotation_test, save_path=None, ratio=0.8, with_plot=True):
    ...
    label_counts = df_combined.groupby(['LABEL', 'Dataset']).size().reset_index(name='Counts')
    gender_counts = df_combined.groupby(['GENDER', 'Dataset']).size().reset_index(name='Counts')
    ...
    if with_plot:
        ...
        sns.barplot(x='LABEL', y='Ratio', hue='Dataset', data=label_counts, ax=axes[0], palette='viridis')
        axes[0].set_title('Label Distribution Ratio')
        axes[0].set_ylabel('Ratio')

        # Plotting the Gender Distribution as ratios
        sns.barplot(x='GENDER', y='Ratio', hue='Dataset', data=gender_counts, ax=axes[1], palette='coolwarm')
        ...
        sns.histplot(df_train['LYMPH_COUNT'], kde=True, ax=axes[2], color='skyblue', label='Train')
        sns.histplot(df_val['LYMPH_COUNT'], kde=True, ax=axes[2], color='orange', label='Validation')
